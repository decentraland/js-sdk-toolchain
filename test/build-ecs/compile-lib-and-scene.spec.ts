...
describe('integration flow, build libs and build scene using libs', () => {
  ...
  itExecutes(
    'npm install --quiet --no-progress -B ' + JSON.stringify(ecsLibCwd),
    sceneCwd
  )
  itExecutes(
    'npm install --quiet --no-progress -B ' + JSON.stringify(rollupLibCwd),
    sceneCwd
  )
  ...
  describe('build-ecs: build scene with library DEBUG mode', () => {
    ...
    it('ensure it uses NON MINIFIED versions in .lib', () => {
      const lib: any[] = JSON.parse(
        readFileSync(resolve(sceneCwd, 'bin/game.js.lib')).toString()
      ).map(($: { path: string }) => resolve(sceneCwd, $.path))
      expect(lib).toContain(resolve(ecsLibCwd, 'bin/lib.js'))
      expect(lib).toContain(resolve(rollupLibCwd, 'dist/index.js'))
      expect(lib).toContain(
        resolve(sceneCwd, 'node_modules/eth-connect/eth-connect.js')
      )
    })
  })
...
describe('integration flow with bundledDependencies, build libs and build scene using libs', () => {
  ...
  describe('build-ecs: build scene with library DEBUG mode', () => {
    ...
    it('ensure it uses NON MINIFIED versions in .lib', () => {
      ...
      expect(lib).toContain(
        resolve(sceneCwd, 'node_modules/@dcl/ecs-scene-utils/dist/index.js')
      )
      expect(lib).toContain(
        resolve(sceneCwd, 'node_modules/eth-connect/eth-connect.js')
      )
