...
export const validateWorkspaceAndWorld = async (
  ...
  if (!isLocalTarget && !worldName && !baseParcel) {
    throw new CliError(
  ...
  const parcels = sceneJson.scene?.parcels ?? (baseParcel ? [baseParcel] : [])

  return { worldName, baseParcel: baseParcel ?? '0,0', parcels }
}
