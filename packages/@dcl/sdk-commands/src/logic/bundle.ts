...
// Pre-register every composite-borne custom component schema in the scene
// at module-load time (pre-seal), so runtime engine.addEntityFromComposite
// calls find each composite's components already defined and don't trip the
// engine's seal. The virtual module is generated by the compositeLoader
// esbuild plugin; an empty file is emitted for scenes with no composites.
import '~sdk/composite-components'
...
function compositeLoader(components: BundleComponents, options: SingleProjectOptions): esbuild.Plugin {
  ...
  return {
    ...
    setup(build) {
      ...
      // Handle composite-borne component pre-registration virtual module.
      // Emitted at module-load (pre-seal) so runtime addEntityFromComposite
      // calls find every composite's components already defined and don't
      // hit `engine.defineComponentFromSchema`'s seal check.
      build.onResolve({ filter: /~sdk\/composite-components/ }, (_args) => {
        return {
          namespace: 'sdk-composite-components',
          path: 'composite-components'
        }
      })

      build.onLoad({ filter: /.*/, namespace: 'sdk-composite-components' }, async (_) => {
        // ensure compositeData is loaded
