...
let entityClikedMap: Map<EventEntityTypeKey, number> = new Map<EventEntityTypeKey, number>()
...
export function wasEntityClicked(entity: Entity, actionButton: ActionButton): boolean {
  ...
  const entityClickedLastCheckTimestamp = getLastTimestamp(entityClikedMap, entity,actionButton, PointerEventType.UP)
  // If the DOWN command has happen before the UP commands, it means that that a clicked has happen
  if (down.timestamp < up.timestamp && up.timestamp > entityClickedLastCheckTimestamp) {
    setLastTimestamp(entityClikedMap,entity,actionButton, PointerEventType.UP, up.timestamp)
    return true // clicked
  }
  return false
}

let entityPointerActiveMap: Map<EventEntityTypeKey, number> = new Map<EventEntityTypeKey, number>()
export function isPointerEventActive(entity: Entity, actionButton: ActionButton, pointerEventType: PointerEventType): boolean {
  ...
  const entityClickedLastCheckTimestamp = getLastTimestamp(entityPointerActiveMap, entity,actionButton, PointerEventType.UP)

  if (command.timestamp > entityClickedLastCheckTimestamp) {
    setLastTimestamp(entityPointerActiveMap,entity,actionButton, PointerEventType.UP, command.timestamp)
...
function setLastTimestamp(map: Map<EventEntityTypeKey, number>, entityId: number, actionButton: ActionButton, pointerEventType: PointerEventType, timestamp: number){
...
function getLastTimestamp(map: Map<EventEntityTypeKey, number>, entityId: number, actionButton: ActionButton, pointerEventType: PointerEventType){
  const eventKey: EventEntityTypeKey = {
    entityId: entityId,
    actionButton: actionButton,
    pointerEventType: pointerEventType
