...
describe('Camera manager', () => {
  ...
  describe('when changing camera speed via mouse input', () => {
    ...
    let mouseDown = {type: BABYLON.PointerEventTypes.POINTERDOWN, event: {pointerType: "mouse"}} as unknown as BABYLON.PointerInfo
    let mouseWheelUp = {type: BABYLON.PointerEventTypes.POINTERWHEEL, event: {deltaY: -240}} as unknown as BABYLON.PointerInfo
    let mouseWheelDown = {type: BABYLON.PointerEventTypes.POINTERWHEEL, event: {deltaY: 240}} as unknown as BABYLON.PointerInfo         

    beforeEach(() => {
      cameraManager = new CameraManager(scene, document.createElement('canvas'), [0, 1, 2], 1, 1, 1)
      mouseDown = {type: BABYLON.PointerEventTypes.POINTERDOWN, event: {pointerType: "mouse"}} as unknown as BABYLON.PointerInfo
      mouseWheelUp = {type: BABYLON.PointerEventTypes.POINTERWHEEL, event: {deltaY: -240}} as unknown as BABYLON.PointerInfo
      mouseWheelDown = {type: BABYLON.PointerEventTypes.POINTERWHEEL, event: {deltaY: 240}} as unknown as BABYLON.PointerInfo         
    })
