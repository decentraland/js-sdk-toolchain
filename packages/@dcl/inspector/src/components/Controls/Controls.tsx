...
const Controls = ({
  ...
  const handleAction = (cb: (e: React.MouseEvent) => void) => (e: React.MouseEvent) => {
    setActive(false)
    cb(e)
  }
  ...
  return (
    <div className={`component controls ${getActiveClass(active)}`}>
      {someActionIsEnabled && <div onClick={handleClick}><RxListBullet /></div>}
      {shouldRenderActions && (
        <div>
          {enableEdit && <button onClick={handleAction(handleEdit)}>
            <MdOutlineDriveFileRenameOutline /> Rename
          </button>}
          {enableAdd && <button onClick={handleAction(handleAdd)}>
            <AiFillFileAdd /> Add child
          </button>}
          {enableRemove && <button onClick={handleAction(handleRemove)}>
            <AiFillDelete /> Delete
          </button>}
        </div>
      )}
