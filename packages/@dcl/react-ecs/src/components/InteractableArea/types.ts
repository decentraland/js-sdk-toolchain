 ...
 * via `UiCanvasInformation` (the region of the canvas not covered by the
 * explorer's HUD — chat, minimap, etc.) and constrains its children to that
 * area using absolute positioning. Layout props that control the container's
 * own position (`positionType`, `position`) are owned by the component and
 * are not configurable from props — every other layout, background and event
