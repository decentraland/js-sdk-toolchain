...
export function entityUtils(engine: IEngine, profile: IProfile | undefined, promiseProfile: Promise<IProfile>) {
  ...
  function syncEntity(entityId: Entity, componentIds: number[], entityEnumId?: number) {
    ...
    // We need the profile for entities created by the client
    if (!profile?.networkId && !entityEnumId) {
      console.log('[Profile not found]')
      void promiseProfile.then(($) => {
        // eslint-disable-next-line no-param-reassign
        profile = $
        console.log('[Profile not found] syncing entity')
        return syncEntity(entityId, componentIds, entityEnumId)
      })
    ...
    const networkValue = { entityId, networkId: profile?.networkId ?? 0 }
