 ...
 */
export const eqPrimitive = (value: Primitive, value2: Primitive): boolean =>
  value === value2
 ...
 */
export const isPrimitiveEqualTo = (
  val: Primitive,
  val2: any
): val2 is Primitive => isPrimitive(val2) && eqPrimitive(val, val2)
 ...
 * will return true
 */
export const hasPrimitiveKeys = <T extends Dict>(
  ...
  for (const [key, value] of Object.entries(comparator)) {
    ...
    if (originalValue === undefined) {
      ...
      isPrimitive(value) &&
      !isPrimitiveEqualTo(value, originalValue)
    ) {
      return false
      ...
      if (Array.isArray(originalValue)) {
        ...
        const foundPrimitive = value.some(
          (val) => isPrimitive(val) && isPrimitiveEqualTo(val, originalValue)
        )
        if (!foundPrimitive) {
          return false
        }
