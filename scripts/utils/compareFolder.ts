...
export function compareFolders(folderA: string, folderB: string) {
  ...
  for (const file of filesA) {
    ...
    if (fileBIndex === -1) {
      ...
      if (fileContentA !== fileContentB) {
        console.error(`The file ${file} has a difference. Please run \`make build-components\` and commit it.`)
        ok = false
      }
    }
  }
  ...
  if (filesB.length > 0) {
    console.error(`The folder ${folderB} has the next files but ${folderA} doesn't: ${filesB.toString()}`)
    ok = false
  }
  if (filesInAButB.length > 0) {
    console.error(`The folder ${folderA} has the next files but ${folderB} doesn't: ${filesInAButB.toString()}`)
    ok = false
  }
